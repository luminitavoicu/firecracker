...
use crate::token::{TokenStore, PATH_TO_TOKEN};
...
lazy_static! {
    ...
    pub static ref TOKENSTORE: Arc<Mutex<TokenStore>> = Arc::new(Mutex::new(TokenStore::default()));
}
...
fn respond_to_get_request_checked(request: Request, custom_headers: CustomHeaders) -> Response {
    ...
    match TOKENSTORE.lock().expect("Poisoned lock").is_valid(token) {
