...
impl TokenTTL {
    ...
    fn new(ttl_seconds: u32) -> Self {
        TokenTTL {
            ttl_seconds: Duration::from_secs(u64::from(ttl_seconds)),
    ...
    fn has_expired(&self) -> bool {
        self.start_time.elapsed().as_secs() >= self.ttl_seconds.as_secs()
    }

    /// Validate the token time to live against bounds.
    fn check_ttl(ttl: u32) -> bool {
...
impl Token {
    ...
    fn generate(ttl_seconds: u32) -> Result<Self, TokenError> {
        ...
        while token.len() < TOKEN_LEN {
            ...
            let mut random_vec = random_bytes.to_vec();
            // Filter out non alphanumeric characters.
            random_vec.retain(|byte| byte.is_ascii_alphanumeric());
    ...
    /// Specifies whether the token has expired.
    fn has_expired(&self) -> bool {
        self.token_info.has_expired()
    }

    /// Get token string sequence.
