...
impl Env {
    ...
    fn exec_into_new_pid_ns(&mut self, chroot_exec_file: PathBuf) -> Result<()> {
        ...
        let mut fds = vec![0; 2];
        // Set up a pipe for the parent-child inter-process communication.
        SyscallReturnCode(unsafe { libc::pipe(fds.as_mut_ptr()) })
            .into_empty_result()
            .map_err(Error::Pipe)?;
        ...
        match pid {
            0 => {
                let mut jailer_time_cpu_us = [0u8; std::mem::size_of::<u64>()];
                // Close write descriptor of the child.
                SyscallReturnCode(unsafe { libc::close(fds[1]) })
                    .into_empty_result()
                    .map_err(Error::ClosePipeFd)?;
                // Read jailer's CPU time from the read end of the pipe.
                SyscallReturnCode(unsafe {
                    libc::read(
                        fds[0],
                        &mut jailer_time_cpu_us as *mut _ as *mut libc::c_void,
                        type_size,
                    )
                } as i32)
                .into_empty_result()
                .map_err(Error::WriteToPipe)?;
                println!(
                    "[child] jailer start time {:?}",
                    u64::from_be_bytes(jailer_time_cpu_us)
                );
                // Reset process start time.
                self.start_time_cpu_us = 0;
                // Set jailer CPU time.
                self.jailer_time_cpu_us = u64::from_be_bytes(jailer_time_cpu_us);
                // Close read descriptor of the child.
                SyscallReturnCode(unsafe { libc::close(fds[0]) })
                    .into_empty_result()
                    .map_err(Error::ClosePipeFd)?;
            ...
            child_pid => {
                ...
                let jailer_time_cpu_us =
                    utils::time::get_time_us(utils::time::ClockType::ProcessCpu)
                        - self.start_time_cpu_us;
                // Close read descriptor of the parent.
                SyscallReturnCode(unsafe { libc::close(fds[0]) })
                    .into_empty_result()
                    .map_err(Error::ClosePipeFd)?;
                // Write jailer's total CPU time to the write end of the pipe.
                SyscallReturnCode(unsafe {
                    libc::write(
                        fds[1],
                        jailer_time_cpu_us.to_be_bytes().as_mut_ptr() as *const libc::c_void,
                        type_size,
                    )
                } as i32)
                .into_empty_result()
                .map_err(Error::WriteToPipe)?;
                println!("[parent] jailer_time_cpu_us {}", jailer_time_cpu_us);
                // Close write descriptor of the parent.
                SyscallReturnCode(unsafe { libc::close(fds[1]) })
                    .into_empty_result()
                    .map_err(Error::ClosePipeFd)?;
    ...
    fn save_exec_file_pid(&mut self, pid: i32, chroot_exec_file: PathBuf) -> Result<()> {
        let pid_file_path = PathBuf::from(format!(
            "{}{}",
            chroot_exec_file
                .to_str()
    ...
    fn exec_command(&self, chroot_exec_file: PathBuf) -> io::Error {
        Command::new(chroot_exec_file)
            ...
            .args(&["--jailer-time-cpu-us", &self.jailer_time_cpu_us.to_string()])
            .stdin(Stdio::inherit())
